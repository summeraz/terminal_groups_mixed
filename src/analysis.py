...
class ProjectAnalysis(FlowProject):
    ...
    def S2_shear_calculated(job):
        return (all([job.isfile('shear_{}nN-S2.txt'.format(load))
                for load in loads]) and 'S2_bottom' in job.document
                and 'S2_top' in job.document)

    @staticlabel()
    def tilt_shear_calculated(job):
        return (all([job.isfile('shear_{}nN-tilt.txt'.format(load))
                for load in loads]) and 'tilt_bottom' in job.document
                and 'tilt_top' in job.document)

    @staticlabel()
    def interdigitation_calculated(job):
        return (all([job.isfile('shear_{}nN-interdigitation.txt'.format(load))
                for load in loads]) and 'interdigitation' in job.document)
    ...
    def counted_h_bonds(job):
        '''
        if any([tg in h_bonding_groups for tg in job.sp['terminal_groups']]):
    ...
    def interaction_energy_logged_5nN(job):
        return 'shear_5nN-Etotal' in job.document
    ...
    def interaction_energy_logged_15nN(job):
        return 'shear_15nN-Etotal' in job.document
    ...
    def interaction_energy_logged_25nN(job):
        return 'shear_25nN-Etotal' in job.document

    '''
    Other properties to possibly calculate:
        - Roughness
        - Gauche defects in the chain backbone
        - Interdigitation
        - H-bonds
