...
groups = ['amino', 'carboxyl', 'hydroxyl']
...
for job in project:
    ...
    if tg1 not in groups and tg2 not in groups:
        for load in [5, 15, 25]:
            all_x.append(job.document['interdigitation']['{}nN'.format(load)])
            cof = job.document['COF']
            intercept = job.document['intercept']
            ff = cof * load + intercept
            all_y.append(ff)
...
ax.set_ylabel('Friction force,nN')
plt.legend()
plt.tight_layout()
plt.savefig('ff-interdigitation-nohbond.pdf')
